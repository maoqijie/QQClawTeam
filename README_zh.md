...
如果您通过 QQ 私聊主机器人，可以直接切换当前私聊会话的模型配置，下一条消息立即生效：
...
本仓库已经内置 `qq-bridge` channel，适合你自己维护 QQ 连接器，把 `NanoClaw` 只当成 Agent 核心来用。

- 入站：你的 QQ 连接器把规范化后的消息 `POST` 到 `http://127.0.0.1:8787/qq-bridge/inbound`
- 出站：`NanoClaw` 会把回复 `POST` 到你配置的 `QQ_BRIDGE_OUTBOUND_URL`
- 私聊默认自动注册；群聊默认需要显式注册，避免误接管陌生群
- 群聊支持 `mentionsSelf=true` 或 `QQ_BRIDGE_COMMAND_PREFIXES` 自动转换为 `@Andy` 触发词

详细协议见 `docs/qq-bridge.md`。
