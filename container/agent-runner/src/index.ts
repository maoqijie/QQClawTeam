...
async function runClaudeBackend(containerInput: ContainerInput): Promise<void> {
  ...
  prompt = `[Tooling note] Treat phrases like “新开一个会话”, “重新开始”, “从头聊”, and “开个新上下文” as requests to clear the current NanoClaw session context, unless the user explicitly says they also want stored history removed.\n\n${prompt}`;
  ...
  prompt = `[Tooling note] If the user asks to clear project memory, reset project memory, or reset CLAUDE/project memory for the current chat, use the available nanoclaw tools to clear project memory. This should reset the current project's CLAUDE.md baseline and clear project auto-memory, while keeping stored chat history intact unless the user explicitly asks for a wipe.\n\n${prompt}`;
