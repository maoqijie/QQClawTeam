...
## QQ self-hosted connector

This fork includes a built-in `qq-bridge` channel so you can keep QQ login, keepalive and rate control in your own connector while NanoClaw remains the agent core.

- Inbound: your QQ connector posts normalized messages to `http://127.0.0.1:8787/qq-bridge/inbound`
- Outbound: NanoClaw posts replies to `QQ_BRIDGE_OUTBOUND_URL`
- Private chats auto-register by default; group chats require explicit registration by default
- Group mentions or configured command prefixes are rewritten to `@Andy` triggers automatically

See `docs/qq-bridge.md` for the request and response contract.
